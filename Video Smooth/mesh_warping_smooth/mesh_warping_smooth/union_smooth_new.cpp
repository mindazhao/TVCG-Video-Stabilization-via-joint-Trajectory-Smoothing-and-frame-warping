...
void union_smooth::warping_video(double fps, bool border)
{
	...
	{
		cv::VideoWriter vw;
		vw.open(save_dest_video_path.substr(0, save_dest_video_path.length()-4)+".avi", CV_FOURCC('X', 'V', 'I', 'D'),
			fps,
			cv::Size(w + 240, h + 240),
			true);
		...
		vw_mesh.open(save_cut_dst_video_path.substr(0, save_dest_video_path.length() - 4) + ".avi", CV_FOURCC('X', 'V', 'I', 'D'),
			fps,
			cv::Size(w + 240, h + 240),
			true);
